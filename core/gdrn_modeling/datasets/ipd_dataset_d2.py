...
class IPD_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/ipd/
        assert osp.exists(self.dataset_root), self.dataset_root
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/ipd/models
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.ipd_full.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.ipd_full.obj2id[obj_name]:06d}.ply"),
...
def get_ipd_metadata(obj_names, ref_key):
...
SPLITS_IPD = dict(
    ipd_train=dict(
        name="ipd_train",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/models"),
        objs=IPD_OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/image_set/{}_{}.txt".format(_obj, "train"))
    ...
    ipd_test=dict(
        ...
        objs=IPD_OBJECTS,
        ann_files=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/image_set/{}_{}.txt".format(_obj, "test"))
            for _obj in IPD_OBJECTS
        ],
        # NOTE: scene root
        image_prefixes=[
...
# single obj splits for ipd real
for obj in ref.ipd_full.objects:
    for split in ["train", "test", "all"]:
        name = "ipd_real_{}_{}".format(obj, split)
        ann_files = [osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/image_set/{}_{}.txt".format(obj, split))]
        ...
        if name not in SPLITS_IPD:
            SPLITS_IPD[name] = dict(
                name=name,
                dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/"),
                models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/models"),
                objs=[obj],  # only this obj
                ann_files=ann_files,
                image_prefixes=[osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/test/{:06d}").format(ref.ipd_full.obj2id[obj])],
                xyz_prefixes=[
                    osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/test/xyz_crop/{:06d}".format(ref.ipd_full.obj2id[obj]))
                ],
                scale_to_meter=0.001,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_IPD:
        used_cfg = SPLITS_IPD[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, IPD_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="ipd",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_ipd_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_IPD.keys())
