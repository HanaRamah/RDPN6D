...
def show_nearest_rotation(pred_views, test_crop, view):
    ...
    cv2.imshow("nearest_views", cv2.resize(nearest_views / 255.0, (len(pred_views) * 256, 256)))
...
def plot_scene_with_3DBoxes(scene_res_dirs, dataset_name="tless", scene_id=1, save=False):
    ...
    for view in range(len(scene_infos)):
        ...
        for bb in scene_gts[view]:
            ...
            cv2.putText(box_img, "%s" % (bb["obj_id"]), (xmin, ymax + 20), cv2.FONT_ITALIC, 0.5, (0, 255, 0), 2)
...
def plot_scene_with_estimate(
    ...
    cv2.putText(scene_view, "%s: %1.3f" % (obj_id, test_score), (xmin, ymax + 20), cv2.FONT_ITALIC, 0.5, (0, 255, 0), 2)
    ...
    cv2.putText(
        scene_view_refined, "%s: %1.3f" % (obj_id, test_score), (xmin, ymax + 20), cv2.FONT_ITALIC, 0.5, (0, 255, 0), 2
    )
...
def compute_pca_plot_embedding(eval_dir, z_train, z_test=None, save=True):
    ...
    if z_test is not None:
        ax.scatter(
            full_z_pca_test[:, 0], full_z_pca_test[:, 1], full_z_pca_test[:, 2], c="red", marker=".", label="test_z"
        )
...
def plot_R_err_hist(eval_args, eval_dir, scene_ids):
    ...
    data_params = dataset_params.get_dataset_params(
        dataset_name, model_type="", train_type="", test_type=cam_type, cam_type=cam_type
    )
...
def plot_vsd_err_hist(
    ...
    tikz_save(out_file, figurewidth="0.45\\textheight", figureheight="0.45\\textheight", show_info=False)


# def plot_vsd_occlusion(eval_args, eval_dir, scene_ids, all_test_visibs, bins=10):
def plot_vsd_occlusion(eval_dir, scene_ids, all_test_visibs, top_n=1, delta=15, tau=20, cost="step", bins=10):
...
def plot_re_rect_occlusion(eval_dir, scene_ids, all_test_visibs, bins=10, top_n=1):
    ...
    for scene_id in scene_ids:
        ...
        if not os.path.exists(os.path.join(eval_dir, "error=re_ntop=%s" % top_n, "errors_{:02d}.yml".format(scene_id))):
            print(
                "WARNING: "
                + osp.join(eval_dir, "error=re_ntop=%s" % top_n, "errors_{:02d}.yml".format(scene_id))
                + " not found"
            )
            continue

        angle_errs_dict = inout.load_yaml(
            os.path.join(eval_dir, "error=re_ntop=%s" % top_n, "errors_{:02d}.yml".format(scene_id))
        )
