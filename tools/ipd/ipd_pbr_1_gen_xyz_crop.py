...
data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/ipd/train_pbr"))

cls_indexes = sorted(idx2class.keys())
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
ipd_model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/ipd/models"))
model_paths = [osp.join(ipd_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None
...
class XyzGen(object):
    def __init__(self, split="train", scene="8"):
        if split == "train":
            scene_ids = scenes
            data_root = data_dir
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen ipd train_pbr xyz")
    ...
    setproctitle.setproctitle(f"gen_xyz_ipd_train_pbr_{args.split}_{args.scene}")
